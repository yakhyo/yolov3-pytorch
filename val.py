...
def run(
    ...
    if not training:
        ...
        dataloader = create_dataloader(
            data[task], imgsz, batch_size, stride, prefix=colorstr(f"{task}: ")
        )[0]
    ...
    for batch_i, (images, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out):
            ...
            predn = pred.clone()
            scale_coords(
                images[si].shape[1:], predn[:, :4], shape, shapes[si][1]
            )  # native-space pred
            ...
            if nl:
                ...
                scale_coords(
                    images[si].shape[1:], tbox, shape, shapes[si][1]
                )  # native-space labels
        ...
        if plots and batch_i < 3:
            ...
            Thread(
                target=plot_images, args=(images, targets, paths, f, names), daemon=True
            ).start()
            f = f"./weights/val_batch{batch_i}_pred.jpg"  # predictions
            Thread(
                target=plot_images,
                args=(images, output_to_target(out), paths, f, names),
                daemon=True,
            ).start()
...
def parse_opt():
    ...
    LOGGER.info(
        colorstr(f"{FILE.stem}: ") + ", ".join(f"{k}={v}" for k, v in vars(opt).items())
    )
    return opt
