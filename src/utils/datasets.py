...
def create_dataloader(path, image_size, batch_size, stride, hyp=None, augment=False, rank=-1, workers=16, prefix='',
                      shuffle=False):
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(path, image_size=image_size,
...
class LoadImagesAndLabels(Dataset):
    ...
    def load_mosaic(self, idx):
        ...
        # Augment
        image4, labels4 = random_perspective(image4, labels4,
...
def random_perspective(image, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    # targets = [cls, xyxy]
