...
from src.utils.general import (box_iou, check_img_size, coco80_to_coco91_class, colorstr, increment_path,
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            if is_coco:
                eval.params.imgIds = [int(Path(x).stem) for x in
                                      dataloader.dataset.image_files]  # image IDs to evaluate
            eval.evaluate()
            eval.accumulate()
            eval.summarize()
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='weights/weights/last.pt', help='model.pt path(s)')
